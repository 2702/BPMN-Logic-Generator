...
public class SplitPattern extends AbstractPattern {
	...
	protected boolean isWellMarked(Node node) {
		if (node.getMarker().equals(Marker.SPLIT) || node.getMarker().equals(Marker.UNTOUCHED)
				|| node.getMarker().equals(Marker.SPLIT_ROOT) || node.getMarker().equals(Marker.SPLIT_TEMP_ROOT))
			return true;
		else
			return false;
	}
	...
	public ArrayList<Node> mark(Node node) {
		...
		if (this.isWellMarked(node) && node.getOutNodes().size() == 2) {
			...
			// po sprawdzonych warunkach ustalamy markery dla glownego jako
			// korzen splita nastepnie wrzucamy to wszystko do listy do
			// zwrocenia
	...
	public boolean isTurnable(ArrayList<Node> nodelist) {
		if (nodelist.get(0).getMarker().equals(Marker.SPLIT_ROOT) || nodelist.get(0).getMarker().equals(Marker.SPLIT_TEMP_ROOT)
				&& nodelist.get(1).getMarker().equals(Marker.SPLIT) && nodelist.get(2).getMarker().equals(Marker.SPLIT))
			return true;
		else
			return false;
	}
